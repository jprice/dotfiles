...
def get_args():
    ...
    parser.add_argument('-d', type=str, help='The interface ID whose MAC 
you wish to change')
...
def set_mac(dev, mac):
    cmd = 'sudo ifconfig {} down'.format(dev)
    subprocess.call(cmd.split())
    cmd = 'sudo ifconfig {} hw ether {}'.format(dev, mac)
    subprocess.call(cmd.split())
    cmd = 'sudo ifconfig {} up'.format(dev)
    subprocess.call(cmd.split())
