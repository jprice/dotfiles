...
city = 'Decatur'
state = 'TN'
secrets = YAML.load_file(ENV['HOME']+'/.dotfiles/conky/secrets.yml')
...
3.times do |index|
  ...
  high       << json[index]['high']['fahrenheit']
  low        << json[index]['low']['fahrenheit']
end
...
# conky output
def img_tag(i, x)
  "${image $HOME/.dotfiles/conky/weather_icons/#{@icons[i]}.png -p #{x},270 -s 75x45}"
end
...
out << "#{g20}#{c0}Temperature #{g130}#{c60}#{temp} F\n"
...
out << "${goto 30}#{c60}#{high[0]}/#{low[0]}F${goto 130}#{high[1]}/#{low[1]}F${goto 240}#{high[2]}/#{low[2]}F"
puts out
